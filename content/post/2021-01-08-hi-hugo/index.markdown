...
Библиотека `modeltime` интегрирована с библиотекой `tidymodels`, что позволяет рассматривать ее в рамках единой экосистемы алгоритмов машинного обучения, основанной на принципах *tidy data*. Узнать больше о  `tidymodels` можно на [сайте библиотеки](https://www.tidymodels.org/) и из новой книги [Kuhn & Silge](https://www.tmwr.org/).
...
Рассмотрим фондовые данные по гидрологии в Российской Федерации за 2008-2015 года[^thanks]. Загрузим данные с **GitHub**:
...
Исходные данные содержат 615 849 записей и имеют две переменных: `date` и `level`, отвечающих за дату наблюдения и уровень воды воды в метрах над нулем графика поста.
...
{{< figure src="ts_filtered_ml_initial_blog.png" title="Исходный временной ряд" >}}
...
{{< figure src="ts_ml_train_test_blog.png" title="Пример разделения временного ряда на обучающую и тестовую выборку" >}}
...
{{< spoiler text="**Нажмите чтобы увидеть код для создания и подгонки моделей**" >}}
...
{{< figure src="ts_ml_forecast_blog.png" title="Пример прогнозирования временного ряда на тестовую выборку" >}}
...
{{% callout note %}}
Как видно из предыдущего шага, не все модели в нашем случае имеют достаточно хорошие показатели ошибок (в частности, показатель $R^2$ должен быть близок к единице, остальные показатели должны быть чем меньше, тем лучше), модели 1-4 и 6 можно удалить из-за низкой точности.
{{% /callout %}}
...
{{< figure src="ts_ml_forecast_1_year_blog.png" title="Прогноз временного ряда с использованием нескольких моделей" >}}
