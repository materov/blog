...
![Исходный временной ряд](ts_filtered_ml_initial_blog.jpg)
...
![Разделение временного ряда на обучающую и тестовую выборку](ts_ml_train_test_blog.jpg)
...
![Прогнозирование временного ряда на тестовую выборку](ts_ml_forecast_blog.jpg)
...
Визуаизируем прогноз.

```r
refit_tbl %>%
  modeltime_forecast(h = "1 year", actual_data = fire_time_series_tbl) %>% 
  filter(.index > "2014-01-01") %>% 
  # корректировка и прогноз
  #modeltime_refit(fire_time_series_tbl) %>%
  #modeltime_forecast(h = "5 months", actual_data = fire_time_series_tbl) %>%
...
![Скорректированный прогноз для различных моделей](ts_ml_forecast_1_year_blog.jpg)
