...
Подробно о виде и форме маркеров можно прочесть на [сайте библиотеки Leaflet](https://rstudio.github.io/leaflet/markers.html). Сделаем карту так, чтобы при наведении на маркер всплывал адрес пожара, рассмотрев только пожары на транспортных средствах. Разделим маркеры на две категории по цвету, соответствующие пожарам:
...
- со специального облачного сервера shinyapps.io;
...
Мы коснулись лишь небольшой части тех возможностей, которыми обладает библиотека Leaflet, однако даже те небольшие возможности, обзор которых здесь представлен, позволяют существенно упростить визуализацию информации связанной с планированием предотвращения последствий различных чрезвычайных ситуаций.
